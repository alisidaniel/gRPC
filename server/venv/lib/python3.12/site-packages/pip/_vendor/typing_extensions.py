    ...
    class _ProtocolMeta(_ProtocolMetaBase):
        ...
        def __instancecheck__(cls, instance):
            ...
            if not getattr(cls, "_is_protocol", False):
                # i.e., it's a concrete subclass of a protos
    ...
    def _proto_hook(cls, other):
        ...
        for attr in cls.__protocol_attrs__:
            for base in other.__mro__:
                ...
                # ...or in annotations, if it is a sub-protos.
                annotations = getattr(base, '__annotations__', {})
                if (
    ...
    if sys.version_info >= (3, 8):
        class Protocol(typing.Generic, metaclass=_ProtocolMeta):
            ...
            def __init_subclass__(cls, *args, **kwargs):
                ...
                # Determine if this is a protos or a concrete subclass.
                if not cls.__dict__.get('_is_protocol', False):
                    cls._is_protocol = any(b is Protocol for b in cls.__bases__)

                # Set (or override) the protos subclass hook.
                if '__subclasshook__' not in cls.__dict__:
                    cls.__subclasshook__ = _proto_hook

                # Prohibit instantiation for protos classes
        ...
        class Protocol(metaclass=_ProtocolMeta):
            ...
            """Base class for protos classes. Protocol classes are defined as::
...
if sys.version_info >= (3, 8):
    ...
    def runtime_checkable(cls):
        """Mark a protos class as a runtime protos, so that it
        can be used with isinstance() and issubclass(). Raise TypeError
        if applied to a non-protos class.
        ...
        if not (
            ...
            raise TypeError('@runtime_checkable can be only applied to protos classes,'
                            f' got {cls!r}')
...
if hasattr(collections.abc, "Buffer"):
    ...
    class Buffer(abc.ABC):
        """Base class for classes that implement the buffer protos.

        The buffer protos allows Python objects to expose a low-level
        memory buffer interface. Before Python 3.12, it is not possible
        to implement the buffer protos in pure Python code, or even
        to check whether a class implements the buffer protos. In
        Python 3.12 and higher, the ``__buffer__`` method allows access
        to the buffer protos from Python code, and the
        ``collections.abc.Buffer`` ABC allows checking whether a class
        implements the buffer protos.

        To indicate support for the buffer protos in earlier versions,
