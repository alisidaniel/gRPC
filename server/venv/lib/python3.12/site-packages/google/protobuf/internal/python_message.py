...
protos message classes from Descriptor objects at runtime.
...
output by the protos compiler at compile-time.

The upshot of all this is that the real implementation
details for ALL pure-Python protos buffers are *here in
this file*.
"""
...
class GeneratedProtocolMessageType(type):
  ...
  """Metaclass for protos message classes created at runtime from Descriptors.

  We add implementations for all methods described in the Message class.  We
  also create properties to allow getting/setting all fields in the protos
  message.  Finally, we create slots to prevent users from accidentally
  "setting" nonexistent fields in the protos message, which then wouldn't get
  serialized / deserialized properly.

  The protos compiler currently uses this metaclass to create protos
  ...
  # Must be consistent with the protos-compiler code in
  ...
  def __new__(cls, name, bases, dictionary):
    ...
    Args:
      ...
      bases: Base classes of the class we're constructing.
        ...
        it's required by the metaclass protos
      dictionary: The class dictionary of the class we're
        constructing.  dictionary[_DESCRIPTOR_KEY] must contain
        a Descriptor object describing this protos message
        type.
...
def _PropertyName(proto_field_name):
  ...
  of a protos message field.

  Args:
    proto_field_name: The protos message field name, exactly
...
def _AddPropertiesForFields(descriptor, cls):
  """Adds properties for all fields in this protos message type."""
  for field in descriptor.fields:
...
def _AddPropertiesForField(field, cls):
  """Adds a public property for a protos message field.
...
def _AddPropertiesForRepeatedField(field, cls):
  """Adds a public property for a "repeated" protos message field.  Clients
  ...
  def setter(self, new_value):
    raise AttributeError('Assignment not allowed to repeated field '
                         '"%s" in protos message object.' % proto_field_name)
...
def _AddPropertiesForNonRepeatedScalarField(field, cls):
  """Adds a public property for a nonrepeated, scalar protos message field.
...
def _AddPropertiesForNonRepeatedCompositeField(field, cls):
  """Adds a public property for a nonrepeated, composite protos message field.
